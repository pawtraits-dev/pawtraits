-- Partner Account Management Database Schema - Clean Installation
...
-- 6. Create indexes for better performance (only if not exists)
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_partners_email') THEN
        CREATE INDEX idx_partners_email ON partners(email);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_partners_business_type') THEN
        CREATE INDEX idx_partners_business_type ON partners(business_type);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_partners_is_active') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referrals_partner_id') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referrals_client_email') THEN
        CREATE INDEX idx_referrals_client_email ON referrals(client_email);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referrals_expires_at') THEN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_commission_payments_partner_id') THEN
        CREATE INDEX idx_commission_payments_partner_id ON commission_payments(partner_id);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_commission_payments_status') THEN
        CREATE INDEX idx_commission_payments_status ON commission_payments(status);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_commission_payments_period') THEN
        CREATE INDEX idx_commission_payments_period ON commission_payments(payment_period_start, payment_period_end);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_client_orders_client_email') THEN
        CREATE INDEX idx_client_orders_client_email ON client_orders(client_email);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_client_orders_partner_id') THEN
        CREATE INDEX idx_client_orders_partner_id ON client_orders(partner_id);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_client_orders_referral_id') THEN
        CREATE INDEX idx_client_orders_referral_id ON client_orders(referral_id);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_client_orders_is_initial') THEN
        CREATE INDEX idx_client_orders_is_initial ON client_orders(is_initial_order);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referral_analytics_referral_id') THEN
        CREATE INDEX idx_referral_analytics_referral_id ON referral_analytics(referral_id);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referral_analytics_event_type') THEN
        CREATE INDEX idx_referral_analytics_event_type ON referral_analytics(event_type);
    END IF;
END $$;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_referral_analytics_created_at') THEN
...
-- 8. Create updated_at triggers (drop and recreate to handle conflicts)
...
DROP TRIGGER IF EXISTS update_commission_payments_updated_at ON commission_payments;
...
-- 10. Drop and recreate RLS policies
DROP POLICY IF EXISTS "Partners can view their own profile" ON partners;
...
DROP POLICY IF EXISTS "Public can view active partner profiles" ON partners;
CREATE POLICY "Public can view active partner profiles" ON partners
    FOR SELECT USING (is_active = true);

-- Referral policies
DROP POLICY IF EXISTS "Partners can manage their own referrals" ON referrals;
...
DROP POLICY IF EXISTS "Public can view referrals by code" ON referrals;
CREATE POLICY "Public can view referrals by code" ON referrals
    FOR SELECT USING (true);

-- Commission payment policies
DROP POLICY IF EXISTS "Partners can view their own commission payments" ON commission_payments;
...
-- Client order policies
DROP POLICY IF EXISTS "Partners can view orders from their referrals" ON client_orders;
CREATE POLICY "Partners can view orders from their referrals" ON client_orders
    FOR SELECT USING (partner_id IN (SELECT id FROM partners WHERE auth.uid()::text = id::text));

-- Analytics policies
DROP POLICY IF EXISTS "Partners can view analytics for their referrals" ON referral_analytics;
...
DROP POLICY IF EXISTS "Public can insert analytics events" ON referral_analytics;
...
-- 12. Create or replace views for dashboard statistics
...
SELECT 
    ...
    COALESCE(SUM(r.qr_scans), 0) as total_qr_scans
FROM partners p
...
-- 13. Create or replace utility functions
...
-- Success message
SELECT 'Partner account management schema installed successfully!' as status;