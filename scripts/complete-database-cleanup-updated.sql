-- COMPLETE DATABASE CLEANUP - Updated Version
-- Removes ALL test data including auth records
...
UNION ALL
SELECT 'referral_analytics' as table_name, COUNT(*) as count FROM referral_analytics
ORDER BY table_name;
...
-- CREATE TABLE backup_user_interactions AS SELECT * FROM user_interactions;
-- CREATE TABLE backup_qr_code_tracking AS SELECT * FROM qr_code_tracking;
...
-- Clear interaction tracking data (if tables exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_interactions') THEN
        DELETE FROM user_interactions;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'interaction_analytics') THEN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'qr_code_tracking') THEN
...
--    - All user interactions and analytics data
--    - All QR code tracking data
...
--    - All sizes definitions
--    - All country/pricing configurations
--
-- ✅ SAFETY FEATURES:
--    - Shows exactly what will be preserved vs deleted
--    - Handles all foreign key constraints properly
--    - Safe table existence checks for optional tables
--    - Comprehensive verification at the end
--    - Optional backup table creation
--
-- ✅ NEW IN THIS VERSION:
--    - Clears user_interactions, interaction_analytics, platform_analytics
--    - Clears qr_code_tracking data
--    - Clears order_items table
--    - Clears cost tracking tables
--    - Handles optional tables that might not exist
