-- Extended Referral System RLS Policies (FIXED VERSION)
-- Run this INSTEAD of the original extended-referral-rls-policies.sql
-- Fixes the partners authentication to match existing schema pattern
...
USING (
    ...
    EXISTS (
        SELECT 1 FROM user_profiles up
        JOIN customers c ON c.email = referrer_customer_email
        JOIN referrals r ON r.client_email = c.email AND r.status = 'applied'
        WHERE up.user_id = auth.uid()
        AND up.user_type = 'partner'
        AND up.partner_id = r.partner_id
        AND c.id = referrer_customer_id
    )
);

-- ===========================================
-- SECURITY FUNCTIONS FOR RLS (UPDATED)
-- ===========================================
...
DO $$
BEGIN
    RAISE NOTICE '🔒 Extended Referral System RLS Policies (FIXED) created successfully!';
    RAISE NOTICE '✅ Security policies implemented with correct authentication pattern:';
    RAISE NOTICE '   - pre_registration_codes: Admin management, public read for active codes';
    RAISE NOTICE '   - customer_referrals: Owner access via user_profiles, public code lookup, admin oversight';
    RAISE NOTICE '   - customer_credits: Owner access via user_profiles, admin oversight';
    RAISE NOTICE '   - customer_credit_transactions: Owner read via user_profiles, service role write';
    RAISE NOTICE '🛡️  Security functions updated to use user_profiles table';
    RAISE NOTICE '⚡ Constraints added to prevent abuse';
    RAISE NOTICE '';
    RAISE NOTICE '✅ Database setup complete! Ready for application integration.';
    RAISE NOTICE '';
    RAISE NOTICE '📋 Fixed authentication issues:';
    RAISE NOTICE '   - Uses user_profiles table for auth.uid() → customer/partner mapping';
    RAISE NOTICE '   - Matches existing authentication pattern in the app';
    RAISE NOTICE '   - Compatible with getCurrentPartner() and getCurrentCustomer() methods';
END $$;