...
-- Handles potential column conflicts with existing schema
...
-- 4. Check if interaction_stats table exists and drop it if it does (to avoid conflicts)
DROP TABLE IF EXISTS interaction_stats CASCADE;

-- 5. Create interaction_stats table for cached aggregated data
CREATE TABLE interaction_stats (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    image_id UUID NOT NULL REFERENCES image_catalog(id) ON DELETE CASCADE,
    like_count INTEGER DEFAULT 0,
    interaction_share_count INTEGER DEFAULT 0, -- Renamed to avoid conflicts
...
-- 6. Create unique constraint and indexes for interaction_stats
...
CREATE INDEX IF NOT EXISTS idx_interaction_stats_share_count ON interaction_stats(interaction_share_count DESC);
CREATE INDEX IF NOT EXISTS idx_interaction_stats_view_count ON interaction_stats(view_count DESC);

-- 7. Create function to update interaction stats
CREATE OR REPLACE FUNCTION update_interaction_stats(p_image_id UUID)
RETURNS VOID AS $$
BEGIN
    INSERT INTO interaction_stats (image_id, like_count, interaction_share_count, view_count, unique_users, last_interaction_at, updated_at)
    SELECT 
        p_image_id,
        COUNT(CASE WHEN interaction_type = 'like' THEN 1 END) - COUNT(CASE WHEN interaction_type = 'unlike' THEN 1 END) as like_count,
        COUNT(CASE WHEN interaction_type = 'share' THEN 1 END) as interaction_share_count,
    ...
    ON CONFLICT (image_id) DO UPDATE SET
        ...
        interaction_share_count = EXCLUDED.interaction_share_count,
...
-- 8. Create trigger to auto-update stats when interactions change
...
-- 10. Drop and recreate share_platform_stats table to avoid conflicts
DROP TABLE IF EXISTS share_platform_stats CASCADE;

-- 11. Create share_platform_stats table for detailed sharing analytics
CREATE TABLE share_platform_stats (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    image_id UUID NOT NULL REFERENCES image_catalog(id) ON DELETE CASCADE,
    platform TEXT NOT NULL,
    platform_share_count INTEGER DEFAULT 0, -- Renamed to avoid conflicts
    last_shared_at TIMESTAMP WITH TIME ZONE,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 12. Create indexes for share_platform_stats
...
CREATE INDEX IF NOT EXISTS idx_share_platform_stats_count ON share_platform_stats(platform_share_count DESC);

-- 13. Create function to update platform stats
CREATE OR REPLACE FUNCTION update_platform_stats(p_image_id UUID, p_platform TEXT)
RETURNS VOID AS $$
BEGIN
    INSERT INTO share_platform_stats (image_id, platform, platform_share_count, last_shared_at, updated_at)
    SELECT 
        p_image_id,
        p_platform,
        COUNT(*) as platform_share_count,
    ...
    ON CONFLICT (image_id, platform) DO UPDATE SET
        platform_share_count = EXCLUDED.platform_share_count,
...
-- 15. Create RLS policies for user_interactions
...
-- 16. Create RLS policies for stats tables (read-only for users)
...
-- 18. Create helpful views for admin analytics

-- View: Popular images with interaction stats (using renamed columns)
...
SELECT 
    ...
    COALESCE(ist.interaction_share_count, 0) as share_count, -- Map to expected name
...
-- View: Sharing platform breakdown (using renamed columns)
...
SELECT 
    ...
    SUM(sps.platform_share_count) as total_shares,
    AVG(sps.platform_share_count) as avg_shares_per_image,
...
-- Success message
SELECT 'User interactions tracking schema installed successfully! Fixed column conflicts.' as status;