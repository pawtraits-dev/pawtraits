-- =============================================
-- Direct Product Pricing System
-- Product = Medium + Format + Size (e.g., Canvas Square Small)
-- Direct country-specific pricing (no multipliers)
-- =============================================

-- 1. Create sizes table (separate from formats)
CREATE TABLE IF NOT EXISTS public.sizes (
  ...
  width_inches DECIMAL(10,2), -- Optional inches display
...
-- 2. Update products table to include size
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS size_id UUID REFERENCES public.sizes(id);
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS gelato_sku TEXT; -- Gelato's exact SKU

-- Update unique constraint
ALTER TABLE public.products DROP CONSTRAINT IF EXISTS products_medium_format_unique;
ALTER TABLE public.products ADD CONSTRAINT products_medium_format_size_unique UNIQUE (medium_id, format_id, size_id);

-- 3. Replace product_pricing with direct country-specific pricing
...
-- 4. Create trigger to auto-calculate profit margins
...
-- 5. Helper function to create complete product
...
-- 6. Helper function to set pricing for a product in specific country
...
-- RLS Policies for sizes (public read, admin write)
...
COMMENT ON TABLE public.sizes IS 'Print sizes (S, M, L, XL) with dimensions';
COMMENT ON TABLE public.product_pricing IS 'Direct country-specific costs and sale prices per product';
COMMENT ON FUNCTION create_product_with_size IS 'Create product: medium + format + size (e.g., Canvas Square Small)';
COMMENT ON FUNCTION set_product_pricing IS 'Set direct pricing: cost from Gelato, sale price to customers';

-- Example usage:
-- 1. Create product: Canvas Square Small
...
-- 2. Set UK pricing: Gelato charges £15, I charge £35
-- SELECT set_product_pricing(product_id, 'GB', 1500, 500, 3500);

-- 3. Set US pricing: Gelato charges $18, I charge $45  
-- SELECT set_product_pricing(product_id, 'US', 1800, 699, 4500);