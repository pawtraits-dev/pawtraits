-- Insert Cat Breeds with Coat Combinations
-- This script checks if breeds exist and adds them along with their coat combinations
-- Based on data from scripts/catbreeds.md

-- First, let's create a temporary function to help with slug generation
CREATE OR REPLACE FUNCTION generate_breed_slug(breed_name TEXT) 
RETURNS TEXT AS $$
BEGIN
    RETURN LOWER(REPLACE(REPLACE(breed_name, ' ', '-'), '''', ''));
END;
$$ LANGUAGE plpgsql;

-- Create temporary table to store our breed data
CREATE TEMP TABLE temp_cat_breeds (
    name TEXT,
    description TEXT,
    available_coats TEXT
);

-- Insert all the cat breed data
INSERT INTO temp_cat_breeds (name, description, available_coats) VALUES
...
-- Now process each breed
DO $$
DECLARE
    breed_record RECORD;
    existing_breed_id UUID;
    new_breed_id UUID;
    coat_name TEXT;
    found_coat_id UUID;
    breed_popularity INTEGER := 1;
BEGIN
    -- Loop through each breed in our temp table
    FOR breed_record IN SELECT * FROM temp_cat_breeds LOOP
        
        -- Check if breed already exists
        SELECT id INTO existing_breed_id 
        FROM breeds 
        WHERE LOWER(name) = LOWER(breed_record.name) 
        AND animal_type = 'cat';
        
        IF existing_breed_id IS NULL THEN
            -- Breed doesn't exist, insert it
            INSERT INTO breeds (
                name, 
                slug, 
                description, 
                ...
                breed_record.name,
                generate_breed_slug(breed_record.name),
                breed_record.description,
                'cat',
                breed_popularity,
                true
            ) RETURNING id INTO new_breed_id;
            
            RAISE NOTICE 'Added new cat breed: % (ID: %)', breed_record.name, new_breed_id;
            existing_breed_id := new_breed_id;
        ELSE
            RAISE NOTICE 'Breed already exists: % (ID: %)', breed_record.name, existing_breed_id;
        END IF;
        
        -- Now handle the coats for this breed
        -- Split the available_coats string and process each coat
        FOR coat_name IN SELECT TRIM(unnest(string_to_array(breed_record.available_coats, ','))) LOOP
            
            -- Find the coat ID
            SELECT id INTO found_coat_id 
            FROM coats 
            WHERE LOWER(name) = LOWER(coat_name) 
            AND animal_type = 'cat';
            
            IF found_coat_id IS NOT NULL THEN
                -- Check if breed-coat combination already exists
                IF NOT EXISTS (
                    SELECT 1 FROM breed_coats 
                    WHERE breed_id = existing_breed_id 
                    AND coat_id = found_coat_id
                ) THEN
                    -- Add the breed-coat combination
                    INSERT INTO breed_coats (
                        breed_id,
                        coat_id,
                        is_common,
                        is_standard,
                    ...
                    RAISE NOTICE 'Added coat "%" to breed "%"', coat_name, breed_record.name;
                ELSE
                    RAISE NOTICE 'Coat "%" already linked to breed "%"', coat_name, breed_record.name;
                END IF;
            ELSE
                RAISE WARNING 'Coat "%" not found for breed "%"', coat_name, breed_record.name;
            END IF;
        END LOOP;
        
        breed_popularity := breed_popularity + 1;
    END LOOP;
END $$;

-- Clean up
DROP FUNCTION generate_breed_slug(TEXT);
DROP TABLE temp_cat_breeds;

-- Show summary of what was added
SELECT 
    'Summary of Cat Breeds and Coats' as summary;

SELECT 
    b.name as breed_name,
    COUNT(bc.coat_id) as coat_count,
    string_agg(c.name, ', ' ORDER BY c.name) as available_coats
FROM breeds b
LEFT JOIN breed_coats bc ON b.id = bc.breed_id
LEFT JOIN coats c ON bc.coat_id = c.id AND c.animal_type = 'cat'
WHERE b.animal_type = 'cat'
GROUP BY b.name
ORDER BY b.name;