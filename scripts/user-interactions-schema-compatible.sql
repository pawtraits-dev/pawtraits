-- User Interactions Tracking Schema - Compatible with Existing Share Tracking
-- This schema works alongside the existing db/share_tracking.sql implementation
-- Uses completely different column names to avoid conflicts
...
-- 4. Create interaction_analytics table for cached aggregated data (avoiding "stats" naming)
...
-- 9. Create platform_analytics table for detailed sharing analytics (avoiding conflicts)
DROP TABLE IF EXISTS platform_analytics CASCADE;
CREATE TABLE platform_analytics (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    image_id UUID NOT NULL REFERENCES image_catalog(id) ON DELETE CASCADE,
    platform TEXT NOT NULL,
    total_platform_shares INTEGER DEFAULT 0, -- Completely unique column name
...
-- 16. Create helpful views for admin analytics (using completely unique names)

-- View: Popular images with interaction analytics
DROP VIEW IF EXISTS popular_images_analytics;
CREATE VIEW popular_images_analytics AS
SELECT 
    ic.*,
    ...
    COALESCE(ia.total_likes, 0) as like_count,
    COALESCE(ia.total_shares, 0) as share_count,
    COALESCE(ia.total_views, 0) as view_count,
    COALESCE(ia.unique_users, 0) as unique_users,
    ia.last_interaction_at
...
DROP VIEW IF EXISTS platform_sharing_analytics;
CREATE VIEW platform_sharing_analytics AS
SELECT 
    pa.platform,
...
GRANT SELECT ON popular_images_analytics TO authenticated;
GRANT SELECT ON popular_images_analytics TO anon;
GRANT SELECT ON platform_sharing_analytics TO authenticated;
GRANT SELECT ON platform_sharing_analytics TO anon;

-- Success message
SELECT 'User interactions tracking schema installed successfully! Compatible with existing share tracking.' as status;