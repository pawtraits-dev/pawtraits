-- Cloudinary Migration Schema Updates
...
-- Structure: {
--   "print_quality": {"url": "...", "secure_url": "...", "access_type": "signed"},
--   "catalog_watermarked": {"url": "...", "watermark_applied": true},
--   "social_optimized": {
--     "instagram_post": "...", 
--     "instagram_story": "...",
--     "facebook_post": "...",
...
-- 3. Update the image_catalog_with_details view to include new fields
...
BEGIN
  ...
  VALUES (
    ...
    p_partner_id,
    jsonb_build_object(
      'landing_url', CONCAT(current_setting('app.qr_base_url', true), '/shop/', p_image_id::text),
      'partner_id', COALESCE(p_partner_id::text, ''),
      'discount_code', CONCAT('PARTNER', current_setting('app.qr_discount', true))
    ),
    1,
    NOW()
  )
...
-- 6. Add RLS policies for QR tracking (if RLS is enabled)
