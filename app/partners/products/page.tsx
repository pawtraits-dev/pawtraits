...
import { Search, Filter, Package, Loader2, DollarSign, Tag } from 'lucide-react';
...
const PARTNER_DISCOUNT_PERCENTAGE = 20; // 20% discount for partners
...
export default function PartnerProductsPage() {
  ...
  const getProductPrice = (product: ProductWithDetails) => {
    if (!product.pricing || product.pricing.length === 0) {
      return { price: null, originalPrice: null, currency: null, symbol: null };
    }
    ...
    if (countryPricing.length === 0) {
      ...
      pricing = countryPricing[0];
    }

    const originalPrice = pricing.sale_price;
    const partnerPrice = Math.round(originalPrice * (1 - PARTNER_DISCOUNT_PERCENTAGE / 100));
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-8">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-3xl font-bold text-gray-900 mb-8">Partner Products</h1>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Browse our available print products with exclusive partner pricing
            </p>
            <div className="flex items-center gap-4 mt-2">
              ...
              <div className="flex items-center gap-2 bg-green-100 text-green-800 px-3 py-1 rounded-full">
                ...
                <span className="text-sm font-medium">{PARTNER_DISCOUNT_PERCENTAGE}% Partner Discount</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {filteredAndSortedProducts.length === 0 ? (
          ...
          <>
            ...
            <div className="text-center text-gray-600">
              Showing {filteredAndSortedProducts.length} of {products.length} products with partner pricing
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredAndSortedProducts.map((product) => {
                ...
                return (
                  <Card key={product.id} className="group hover:shadow-lg transition-all duration-200 overflow-hidden border-green-200">
                    <CardContent className="p-6 space-y-4">
                      {/* Featured badge and partner badge at top */}
                          ...
                          <div className="text-center">
                            ...
                            {pricing.originalPrice && pricing.originalPrice > pricing.price && (
                              <div className="text-sm text-gray-500 line-through">
                                {formatPrice(pricing.originalPrice, pricing.currency || 'GBP', pricing.symbol)}
                              </div>
                            )}
                            <div className="text-xs text-green-600 font-medium">
                              Partner Price ({PARTNER_DISCOUNT_PERCENTAGE}% off)
                            </div>
                          </div>
                        ) : (
