...
// Removed direct Supabase client import - using API-only architecture
...
function InfluencerReferralsContent() {
  ...
  const fetchInfluencerData = async () => {
    try {
      ...
      // Fetch referral analytics using API-only architecture
      const analyticsResponse = await fetch('/api/referrals/analytics', {
        credentials: 'include',
        headers: {
      ...
      if (analyticsResponse.ok) {
        ...
        setInfluencerData(analyticsData);

        // Transform analytics to referral list format for table
        const transformedReferrals = analyticsData.recent_activity?.map((activity: any, index: number) => ({
          id: activity.id,
          customer_email: `referral-${index + 1}@privacy.protected`, // Privacy protected
          status: activity.type === 'purchase' ? 'purchased' : 'signed_up',
          source_platform: 'influencer',
          order_value: activity.order_value || 0,
