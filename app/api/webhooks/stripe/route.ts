...
async function handlePartnerCommission(
  ...
  try {
    ...
    // Get partner commission rates via API using correct production URL
    const baseUrl = process.env.VERCEL_URL
      ? `https://${process.env.VERCEL_URL}`
      : process.env.NEXT_PUBLIC_BASE_URL
      || 'http://localhost:3000';

    const partnerResponse = await fetch(`${baseUrl}/api/partners/by-email/${encodeURIComponent(partnerProfile.email)}`);
    ...
    // Use commission API endpoint with correct production URL
    const commissionResponse = await fetch(`${baseUrl}/api/commissions/partner`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        orderId: order.id,
    ...
    if (commissionResponse.ok) {
      ...
      console.log('✅ Partner commission created via API:', {
