  ...
  const loadCatImages = async () => {
    try {
      ...
      if (response.ok) {
        ...
        let catImages = (data.images || []).filter((img: ImageCatalogWithDetails) => 
          img.breed?.animal_type === 'cat' || 
          img.tags?.includes('cat') ||
          (!img.breed?.animal_type && img.tags?.includes('cat')) // Explicit cat tag
        );

        // Apply search filter
        if (searchTerm) {
          catImages = catImages.filter((img: ImageCatalogWithDetails) =>
        ...
        // Apply breed filter
        if (selectedBreed) {
          catImages = catImages.filter((img: ImageCatalogWithDetails) => img.breed_id === selectedBreed);
        }

        // Apply theme filter
        if (selectedTheme) {
          catImages = catImages.filter((img: ImageCatalogWithDetails) => img.theme_id === selectedTheme);
        }

        // Apply featured filter
        if (featuredOnly) {
          catImages = catImages.filter((img: ImageCatalogWithDetails) => img.is_featured);
        }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <StickyFilterHeader
        ...
        searchPlaceholder="Search cat images..."
