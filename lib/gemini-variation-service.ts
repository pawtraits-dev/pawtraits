...
Ensure the ${primaryAnimal.coat.coat_name} coloring is consistent across ALL body parts of the ${primaryAnimal.breed.name}, and the ${secondaryAnimal.coat.coat_name} coloring is consistent across ALL body parts of the ${secondaryAnimal.breed.name}.`;
  ...
  private createBreedVariationPromptWithCoat(
    ...
    // Determine clothing instruction based on whether outfit is being changed
    const clothingInstruction = targetOutfit
      ? `Change the outfit/clothing to ${outfit}.`
      : 'Keep the same clothing.';

    if (isCrossSpecies) {
      // Cross-species transformation prompt
      return `Using the provided image of a ${originalAnimalType} (${originalBreed?.name || breed}) wearing ${parsedOutfit}, transform this ${originalAnimalType} into a ${targetAnimalType} breed: ${targetBreed.name.toLowerCase()}. This is a ${originalAnimalType}-to-${targetAnimalType} transformation. Apply the ${validCoat.coat_name.toLowerCase()} coloring throughout ALL body parts:\n- Face and head fur\n- Body and torso fur\n- All four legs completely\n- All four paws and feet\n- Tail fur\n- Any visible undercoat\n\n${ageInstruction}\n\n${furLengthInstruction}\n\nTransform the anatomy and characteristics completely from ${originalAnimalType} to ${targetAnimalType}: change head shape, ear type, body size, facial features, and tail to match a ${targetBreed.name.toLowerCase()}. Ensure the ${validCoat.coat_name.toLowerCase()} fur color is realistic and consistent for this ${targetBreed.name.toLowerCase()} breed. ${clothingInstruction} Keep the same pose, lighting, and overall composition.`;
    } else {
      // Same species breed variation prompt
      return `Using the provided image of a ${breed.toLowerCase()} wearing ${parsedOutfit}, change the breed to a ${targetBreed.name.toLowerCase()} and change the fur color to ${validCoat.coat_name.toLowerCase()}. Apply the ${validCoat.coat_name.toLowerCase()} coloring throughout ALL body parts:\n- Face and head fur\n- Body and torso fur\n- All four legs completely\n- All four paws and feet\n- Tail fur\n- Any visible undercoat\n\n${ageInstruction}\n\n${furLengthInstruction}\n\nChange the breed characteristics (head shape, ear type, body size, facial features) and ensure the ${validCoat.coat_name.toLowerCase()} fur color is realistic and consistent for this ${targetBreed.name.toLowerCase()} breed. ${clothingInstruction} Keep the same pose, lighting, and overall composition.`;
    }
  }
