...
export class QRCodeService {
  ...
  async generatePawtraitsQRCode(
    ...
    // Use the Pawtraits paw logo (rotated 180 degrees to fix upside-down issue)
...
/**
 * Generate and upload branded QR code for personal referral codes (customers/partners)
 * Uses the same branded style as pre-registration codes
 */
export async function generatePersonalReferralQR(
  referralCode: string,
  type: 'customer' | 'partner',
  baseUrl: string = process.env.NEXT_PUBLIC_SITE_URL || 'https://pawtraits.pics'
): Promise<{ success: boolean; qrCodeUrl?: string; error?: string }> {
  try {
    const qrCodeService = new QRCodeService();

    // Determine URL based on type (customers use /c/, partners use /c/ for referring customers)
    const qrUrl = `${baseUrl}/c/${referralCode}`;
    ...
    // Create file object from buffer
    const qrFile = new File([qrCodeBuffer], `qr-${type}-${referralCode}.png`, {
      type: 'image/png'
    });

    // Upload to Supabase Storage
    const uploadResult = await qrCodeService.supabaseService.uploadImage(qrFile);
    ...
    console.error(`Error generating ${type} personal QR code:`, error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error'
    };
